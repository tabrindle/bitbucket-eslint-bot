...
const eslintDocsBaseUrl =
  "https://github.com/eslint/eslint/blob/master/docs/rules";

const encodeAuthorization = ({ user, password }) => {
  return `Basic ${new Buffer(`${user}:${password}`, "binary").toString(
    "base64"
  )}=`;
};
  ...
  return lintResults.map(file => {
    if (file.messages.length) {
      return file.messages
        .filter(message => {
          return !(!options.warnings && message.severity === 1);
        })
        .map(message => {
          if (options.debug) {
            console.log("POST to: ", url);
            console.log(
              `[eslint] ${message.message} - ([${message.ruleId}](${eslintDocsBaseUrl}/${message.ruleId}.md))`
            );
            console.log({
              line: message.line,
              lineType: "ADDED",
...
module.exports.run = function({
  ...
  try {
    ...
    if (lintResults && lintResults.length) {
      if (commentTopLevel) {
        commentTopLevelFn(lintResults, url, bitbucketUrl, {
          debug,
          user,
          password,
          createTask
      ...
      if (commentFileLevel) {
        commentFileLevelFn(lintResults, url, {
          debug,
          user,
          password,
          warnings
